...
public class CustomAuthenticationManager implements AuthenticationManager {
	...
	private Authentication checkAdmin(String password, User u, Authentication authentication)
		...
		if (password.equals(u.getPassword())) {
			logger.warn("user={}, action={}, status={}", u.getBanner(), "Check Admin User Credentials", "Success");
			...
			logger.info("user={}, action={}, status={}", u.getBanner(), "Create Admin Authentication Token", "Success");
			return token;
		} else {
			logger.warn("user={}, action={}, status={}", u.getBanner(), "Check Admin User Credentials", "Fail");
	...
	private Authentication checkNormal(String password, User u, Authentication authentication)
		...
		if (passwordEncryption.matches(password, u.getPassword())) {
			logger.warn("user={}, action={}, status={}", u.getBanner(), "Check User Credentials", "Success");
			...
			AbstractAuthenticationToken token = secFactory.createAuthenticationToken(authentication.getPrincipal(),
					authentication.getCredentials(), rights);
			logger.info("user={}, action={}, status={}", u.getBanner(), "Create User Authentication Token", "Success");
			return token;
		} else {
			logger.warn("user={}, action={}, status={}", u.getBanner(), "Check User Credentials", "Fail");
			throw new BadCredentialsException("1000");
		}
	}
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		try {
			...
			logger.warn("user={}, action={}, status={}, message={}", CUSTOME_AUTH_MANAGER_LOG, "User Authentication",
					"Fail", e.getMessage());
			throw new AuthenticationServiceException("1000");
		}
		logger.warn("user={}, action={}, status={}", u.getBanner(), "User Authentication", "Starting...");
		if (u.isValidUser()) {
			logger.warn("user={}, action={}, status={}", u.getBanner(), "Check User Validity", "Success");
			...
			logger.warn("user={}, action={}, status={}", u.getBanner(), "Check User Validity", "Fail");
