...
public class BCryptPasswordEncryption implements IPasswordEncryption {
	...
	public BCryptPasswordEncryption() {
		...
		logger.info("password={}, action={}, status={}",
				BCRYPT_ENCRYPTION_LOG, "Create Encoder", "Success");
	}

	public String encryptPassword(String rawPassword) {
		String encoded = encoder.encode(rawPassword);
		logger.info("password={}, action={}, status={}",
				BCRYPT_ENCRYPTION_LOG, "Encode Raw Password", "Success");
		return encoded;
	}

	public boolean matches(String rawPassword, String encryptedPassword) {
		boolean matchResult = encoder.matches(rawPassword, encryptedPassword); 
		logger.info("password={}, action={}, result={}",
				BCRYPT_ENCRYPTION_LOG, "Match Passwords", matchResult);
		return matchResult;
	}
}
