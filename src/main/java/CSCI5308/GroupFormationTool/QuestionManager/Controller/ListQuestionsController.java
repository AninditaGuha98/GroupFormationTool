...
public class ListQuestionsController {
    ...
    public ModelAndView performSort(@ModelAttribute("sorters") Sorters sorters, BindingResult result, ModelMap model) {
        ...
        model.addAttribute("message","Table is sorted by "+sortingFields.get(sorters.getSortField())+" in "+sortingOrders.get(sorters.getSortOrder())+" Order");
    ...
    public ModelAndView performClearSort(@ModelAttribute("sorters") Sorters sorters, BindingResult result, ModelMap model) {
        setModelForSorting(model);
